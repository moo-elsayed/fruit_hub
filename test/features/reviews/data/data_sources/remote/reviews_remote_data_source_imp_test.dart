...
void main() {
  ...
  group('checkUserPurchasedProduct', () {
    ...
    test('should return NetworkSuccess with true when user has a delivered order containing the product', () async {
      // Arrange
      await fakeFirestore.collection(BackendEndpoints.ordersCollection).add({
        'uId': tUserId,
        'status': 'delivered',
        'orderItems': [
    ...
    test('should return NetworkSuccess with false when user order contains product but status is not delivered', () async {
      // Arrange
      await fakeFirestore.collection(BackendEndpoints.ordersCollection).add({
        'uId': tUserId,
        'status': 'pending',
    ...
    test('should return NetworkSuccess with false when user has delivered orders but none contain product', () async {
      // Arrange
      await fakeFirestore.collection(BackendEndpoints.ordersCollection).add({
        'uId': tUserId,
        'status': 'delivered',
    ...
    test('should return NetworkSuccess with false when orders collection has no orders for the user', () async {
      // Arrange
      await fakeFirestore.collection(BackendEndpoints.ordersCollection).add({
  ...
  group('addReview', () {
    ...
    test('should return NetworkSuccess and update product document with new review, avgRating, and ratingCount when product exists', () async {
      // Arrange
      await fakeFirestore
    ...
    test('should correctly calculate avgRating with rounded single decimal place when average is recurring decimal', () async {
    ...
    test('should return NetworkFailure with notFoundError when product document does not exist', () async {
    ...
    test('should return NetworkFailure with unexpectedError when product document has malformed data causing exception', () async {
      // Arrange
      await fakeFirestore
          .collection(BackendEndpoints.productsCollection)
          .doc(tProductCode)
          .set({'name': 'تفاح', 'reviews': 'invalid_reviews_string'});
  ...
  group('Model and Entity Mappings', () {
    ...
    test('ReviewModel fromJson and toJson should serialize and deserialize correctly with conditional userId', () {
      // Arrange
      final jsonWithUser = {
        'name': 'هند',
    ...
    test('ReviewModel fromJson should handle uId alias fallback and missing/null fields with default values', () {
      // Arrange
    ...
    test('FruitModel fromJson should parse complete json correctly with fallbacks and numeric type coercion', () {
      // Arrange
      final json = {
